...
router.get('/tarefas', (req, res) => {
    ...
    }).catch(err => res.send("Erro ao listar tarefas: " + err));
});
...
router.get('/editar_tarefas/:id', (req, res) => {
    ...
    }).catch(err => res.send("Erro ao carregar tarefa: " + err));
});

/*6°) Recebe as informações do botão que está no edittarefa.handlebar e efetua a alteração no banco de dados. Volta para listagem das tarefas*/
router.post('/tarefas/editar_tarefas', (req, res) => {
    Tarefas.updateOne({_id: req.body._id},
        ...
        res.redirect("/rota_tarefas/tarefas");
    }).catch(err => res.send("Erro ao editar tarefa: " + err));
});

/*7°) No form tarefas.handlebars que lista as tarefas possui um botão para deletar, clicando nesse botão ele deleta informação e refaz a lista*/
router.get('/deletar_tarefas/:id', (req, res) => {
    Tarefas.deleteMany({_id: req.params.id}).then(() => {
        res.redirect("/rota_tarefas/tarefas");
    }).catch(err => res.send("Erro ao deletar tarefa: " + err));
});
